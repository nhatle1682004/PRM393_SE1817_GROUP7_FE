...
class _CreateReportScreenState extends State<CreateReportScreen> implements ReportView {
  ...
  @override
  void onLocationUpdated(double lat, double lng) => setState(() { _lat = lat; _lng = lng; });
  @override
  ...
  @override
  void onSubmitting(bool isSubmitting) => setState(() => _isSubmitting = isSubmitting);
  ...
  @override
  void onError(String err) => ScaffoldMessenger.of(context).showSnackBar(
  ...
  AppBar _buildMobileAppBar() => AppBar(
    ...
    elevation: 0,
    leading: IconButton(icon: const Icon(Icons.arrow_back, color: Color(0xFF1E293B)), onPressed: () => Navigator.of(context).pop()),
  ...
  Widget _buildMobileLayout() => Column(children: [
    PhotoCard(isMobile: true, imageFile: _imageFile, webPickedFile: _webPickedFile, selectedTypes: _selectedTypes, onPickImage: () => _presenter.pickImage(), onToggleWasteType: (type) => _presenter.toggleWasteType(type)),
    const SizedBox(height: 16),
    LocationCard(isMobile: true, lat: _lat, lng: _lng, onGetLocation: () => _presenter.getCurrentLocation()),
    const SizedBox(height: 16),
  ...
  Widget _buildTabletLayout() => Column(children: [
    Row(children: [
      Expanded(child: PhotoCard(isMobile: false, imageFile: _imageFile, webPickedFile: _webPickedFile, selectedTypes: _selectedTypes, onPickImage: () => _presenter.pickImage(), onToggleWasteType: (type) => _presenter.toggleWasteType(type))),
      const SizedBox(width: 16),
      Expanded(child: LocationCard(isMobile: false, lat: _lat, lng: _lng, onGetLocation: () => _presenter.getCurrentLocation())),
    ]),
    const SizedBox(height: 16),
  ...
  Widget _buildDesktopLayout() => Column(children: [
    Row(children: [
      Expanded(flex: 6, child: PhotoCard(isMobile: false, imageFile: _imageFile, webPickedFile: _webPickedFile, selectedTypes: _selectedTypes, onPickImage: () => _presenter.pickImage(), onToggleWasteType: (type) => _presenter.toggleWasteType(type))),
      const SizedBox(width: 20),
      Expanded(flex: 5, child: LocationCard(isMobile: false, lat: _lat, lng: _lng, onGetLocation: () => _presenter.getCurrentLocation())),
    ]),
